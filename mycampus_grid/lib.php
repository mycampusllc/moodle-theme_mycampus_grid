...
function theme_mycampus_grid_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'logo') {
        $theme = theme_config::load('mycampus_grid');
 ...
 *
 * Do not add mycampus_grid specific logic in here, child themes should be able to
 ...
 */
function mycampus_grid_process_css() {
 ...
 * @deprecated since 2.5.1
 */
function mycampus_grid_set_logo() {
    throw new coding_exception('Please call theme_'.__FUNCTION__.' instead of '.__FUNCTION__);
}
 ...
 * @deprecated since 2.5.1
 */
function mycampus_grid_set_customcss() {
    throw new coding_exception('Please call theme_'.__FUNCTION__.' instead of '.__FUNCTION__);
}
...
//mycampus_grid_GetCourseTeacherNumber
function mycampus_grid_GetCourseTeacherNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
//mycampus_grid_GetCourseQuestionNumber
function mycampus_grid_GetCourseQuestionNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_GetCourseEnrollmentStudentNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_GetCourseSectionNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_GetCourseQuizNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_GetCourseOneMouthActivesNumber($id)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_substr_for_utf8($sourcestr,$cutlength)
{
	$returnstr="";
	$i=0;
	$n=0;
...
function mycampus_grid_objectToArray($rs_a)				{
...
function mycampus_grid_randpictice($id,$course='course')
{

	$CourseFile = "theme/mycampus_grid/images/".$course."_".$id.".jpg";
	if(is_file($CourseFile))			{
		$returnCourseGroupImg = $CourseFile;
	}
	else	{
		$returnCourseGroupImg = "/theme/mycampus_grid/images/".$course."_0000.jpg";
	}

	return $returnCourseGroupImg;
}

function mycampus_grid_indexcoursedetailshow($html)		{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	...
	for($i=0;$i<$courseNUM;$i++)
	{
		...
		//echo $summary."<br>";
		$returnCourseGroupImg = mycampus_grid_randpictice($id);
		//得到老师信息
		$mycampus_grid_GetCourseTeacherNumber = mycampus_grid_GetCourseTeacherNumber($id);
		mycampus_grid__ReturnTableCourseYindaoye($html,$fullname,$summary,$returnCourseGroupImg,$id,$mycampus_grid_GetCourseTeacherNumber,$shortname);
	}
	print "</td></tr></table>";
}
...
//返回课程引导页输出
function mycampus_grid__ReturnTableCourseYindaoye($html,$fullname,$summary,$returnCourseGroupImg,$id,$mycampus_grid_GetCourseTeacherNumber,$shortname)
{
	global $CFG,$dbA;
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	$indexshowcoursenumber	= $html->indexshowcoursenumber;
	...
	print "
				<script type=\"text/javascript\" src=\"/theme/mycampus_grid/js/jquery-1.7.1.min.js\"></script>
				<script src=\"/theme/mycampus_grid/js/highcharts/highcharts.js\"></script>
				<script src=\"/theme/mycampus_grid/js/highcharts/modules/exporting.js\"></script>
				<style type=\"text/css\">
				${demo.css}
				</style>
				";
	...
	print "<tr><td width=15>&nbsp;</td><td><BR>
		$shortname [教师:".$mycampus_grid_GetCourseTeacherNumber_TEXT."]
		&nbsp;&nbsp;
...
function mycampus_grid_IndexMainCourseShow($html)			{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
	$indexshowcoursenumber	= $html->indexshowcoursenumber;
	$adodbcacheseconds		= $html->adodbcacheseconds;
	print "<link rel=\"stylesheet\" type=\"text/css\" href=\"/theme/mycampus_grid/style/style_".$indexshowcoursenumber.".css\" />";
	//print_R($html);exit;
	if($html->isshowlogo)				{
		...
		if($contenthash!="")			{
			...
			$URL = "/theme/mycampus_grid/pix/logo.jpg";
		}
	...
	if($html->wonderfulcourse&&$html->wonderfulcoursenumber!="")				{
		...
		for($i=0;$i<sizeof($CourseInforArray_ARRAY['id']);$i++)
		{
			...
			$mycampus_grid_GetCourseEnrollmentStudentNumber	= mycampus_grid_GetCourseEnrollmentStudentNumber($id);
			$mycampus_grid_GetCourseQuestionNumber			= mycampus_grid_GetCourseQuestionNumber($id);
			$mycampus_grid_GetCourseQuizNumber				= mycampus_grid_GetCourseQuizNumber($id);
			$mycampus_grid_GetCourseSectionNumber				= mycampus_grid_GetCourseSectionNumber($id);
			$mycampus_grid_GetCourseOneMouthActivesNumber		= mycampus_grid_GetCourseOneMouthActivesNumber($id);
			$mycampus_grid_GetCourseTeacherNumber				= mycampus_grid_GetCourseTeacherNumber($id);

			$CourseNameShowText					= mycampus_grid_substr_for_utf8($fullname,10);
				...
				<li>
				  <div class=\"dandian_info\"> <img src=\"$returnCourseGroupImg\" />
					<div class=\"dandian_text\">
					  <p>".$coursestatistics_teachername.": ".join(',',$mycampus_grid_GetCourseTeacherNumber)."</p>
					  <p>".$coursestatistics_users.": $mycampus_grid_GetCourseEnrollmentStudentNumber</p>
					  <p>".$coursestatistics_sections.": $mycampus_grid_GetCourseSectionNumber</p>
					  <p>".$coursestatistics_quizs.": $mycampus_grid_GetCourseQuizNumber</p>
					  <p>".$coursestatistics_questions.": $mycampus_grid_GetCourseQuestionNumber</p>
					  <p>".$coursestatistics_actives.": $mycampus_grid_GetCourseOneMouthActivesNumber</p>
					</div>
				  </div>
				  <BR>
	...
	if($html->hotcourse)				{
		...
		for($i=0;$i<sizeof($rs_a);$i++)		{
			if($rs_a[$i]['courseid']>1)		{
				...
				$mycampus_grid_GetCourseOneMouthActivesNumberALL[$courseid] = $rs_a[$i]['num'];
			}
		}
		...
		for($i=0;$i<sizeof($CourseInforArray_ARRAY['id']);$i++)
		{
			...
			$mycampus_grid_GetCourseQuizNumber				= mycampus_grid_GetCourseQuizNumber($id);
			$mycampus_grid_GetCourseSectionNumber				= mycampus_grid_GetCourseSectionNumber($id);
			$mycampus_grid_GetCourseOneMouthActivesNumber		= $mycampus_grid_GetCourseOneMouthActivesNumberALL[$id];
			$mycampus_grid_GetCourseTeacherNumber				= mycampus_grid_GetCourseTeacherNumber($id);
...
if($action=="imagesetting"&&$id!="")			{
	if($coursetroup=="group")			{
		$NewCourseImage = "theme/mycampus_grid/images/group_".$id.".jpg";
	}
	else	{
		$NewCourseImage = "theme/mycampus_grid/images/course_".$id.".jpg";
	}
...
function mycampus_grid_coursecontextgroup($html)										{
	global $dbA,$adodbcacheseconds,$indexshowcoursenumber;
...
function mycampus_grid_left_button($html)		{
	//print_R($html);
	print "
